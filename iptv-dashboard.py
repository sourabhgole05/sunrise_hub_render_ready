...
def yf_chart(sym, range_="1d", interval="5m"):
    ...
    for h in YF_HOSTS:
        try:
            ...
            cl = [c for c in (q.get("close") or []) if c is not None]
...
function pushHist(it,tp){if(!it||!it.u)return;
 ...
 S.hist.unshift({n:it.n,u:it.u,l:it.l,g:it.g,tp:tp||'tv'});
...
function sparkV(vals,w,h){
 ...
 return'<svg width="'+w+'" height="'+h+'"><polyline fill="none" stroke="'+col+'" stroke-width="2" points="'+pts.join(' ')+'"/></svg>'}
    ...
    def do_GET(self):
        ...
        if u.path == "/":
            ...
            try:
                ...
                self.send(502, json.dumps({"error": str(exc)[:100]}),
                          "application/json")

        elif u.path == "/api/books/search":
            query = qs.get("q", [""])[0].strip()[:60]
            language = qs.get("lang", ["en"])[0]
            try:
                page = min(50, max(1, int(qs.get("page", ["1"])[0] or 1)))
            ...
            try:
                ...
                pass
            try:
                j = gutendex({"search": query, "languages": language.split(","),
                              "page": page})
                if j.get("results"):
                    self.send(200, json.dumps({"items": j.get("results", []),
                                               "page": page, "source": "gutendex"},
                                              ensure_ascii=True),
                              "application/json")
                    return
            except Exception:
                pass
            self.send(200, json.dumps({"items": [], "page": page,
                                       "source": "none"},
                                      ensure_ascii=True), "application/json")

        elif u.path == "/api/books/gutenberg":
            query = qs.get("q", [""])[0].strip()[:60]
            try:
                j = gutendex({"search": query})
                items = []
                for book in j.get("results", [])[:5]:
                ...
                self.send(200, json.dumps({"results": items}, ensure_ascii=True),
                          "application/json")
            except Exception:
                self.send(200, json.dumps({"results": []}, ensure_ascii=True),
                          "application/json")

        elif u.path == "/api/market/quote/{sym}":
            sym = qs.get("sym", [""])[0][:20]
            if not sym:
                return self.send(400, '{"error":"missing symbol"}',
                                 "application/json")
        ...
        elif u.path == "/api/stock/{ticker}":
            ticker = qs.get("ticker", [""])[0][:20]
            if not ticker:
                return self.send(400, '{"error":"missing ticker"}',
                                 "application/json")
            try:
        ...
        elif u.path == "/api/market/news-feed":
            try:
        ...
        elif u.path == "/api/books/gutenberg/{id}":
            book_id = qs.get("id", [""])[0]
            try:
                j = gutendex({"ids": book_id})
                res = j.get("results", [])
                if not res:
                    raise RuntimeError("book not found")
                book = res[0]
                fmts = book.get("formats") or {}
                cov = fmts.get("image/jpeg", "")
                text_formats = [k for k in fmts if "text/plain" in k]
                if not text_formats:
                    raise RuntimeError("no plain-text edition")
                text_formats.sort(key=lambda k: (0 if "utf-8" in k.lower() else 1))
                text_url = fmts[text_formats[0]]
...
def register_book_routes(app):
    ...
    async def fastapi_books_search(
        ...
        try:
            result = gutendex({"search": search, "languages": selected_language.split(","),
